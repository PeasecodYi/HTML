<template>
  <h1>选配件</h1>
  <el-form :model="feature" label-position="center">
    <el-row :gutter="20">
      <el-col :span="5">
        <el-form-item label="ID:">
          <el-input v-model="feature.id" readonly />
        </el-form-item>
      </el-col>
      <el-col :span="3">
      ...
      <el-form-item>
        <el-button type="success" @click="doAdd(API_URLs.feature.add)">
          <el-icon>
            <Plus />
          </el-icon>
          新增
        </el-button>
        <el-button type="primary" @click="doSave(API_URLs.feature.save)">
          <el-icon>
            <check />
          </el-icon>
          保存
  ...
  <el-row>
    <el-table :data="features" style="width: 100%" height="600px">
      <el-table-column
        prop="id"
        label="ID"
        width="180"
      ...
      <el-table-column
        ...
        align="center"
      />
      <el-table-column prop="caption" label="配件" header-align="center" />
      <el-table-column label="操作" header-align="center" align="center">
        <template #default="scope">
          ...
          <el-button
            type="danger"
            @click="doDelete(scope.row, API_URLs.feature.delete)"
          >
            <el-icon>
              <Delete />
            </el-icon>
...
const API_URLs = {
  feature: {
    ...
    add: `http://wsmooc.jxufe.edu.cn/data/json/carBrand/features/add?token=${token}`,
    save: `http://wsmooc.jxufe.edu.cn/data/json/carBrand/features/save?token=${token}`,
    delete: `http://wsmooc.jxufe.edu.cn/data/json/carBrand/features/delete?token=${token}`,
  },
};
let feature = ref({
  id: 0,
...
const doModify = (row: any) => {
  feature.value = JSON.parse(JSON.stringify(row));
};
